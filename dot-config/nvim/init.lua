...
-- finally we do our own stuff here:
local serverhorror_autocmds = "serverhoror-autocmd"
vim.api.nvim_create_augroup(serverhorror_autocmds, { clear = true })
vim.api.nvim_create_autocmd({ "CursorMoved", "CursorMovedI", "WinScrolled", "BufWinEnter" }, {
    pattern = "*",
    desc = "Keep CursorLine centered",
    group = serverhorror_autcmds,
    callback = function()
        -- vim.cmd('norm! zz')
    end,
})
