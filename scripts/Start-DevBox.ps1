...
function Invoke-WithErrorHandling {
  ...
  if ($LASTEXITCODE -ne 0) {
    ...
    throw "`$Command @Arguments' exited with code $LASTEXITCODE."
  }
}
...
function Start-DevBox {
  ...
  try {
    ...
    # Login to AWS SSO
    Write-Warning "$(Get-TimeStamp) - Could not find a valid session for profile $AWSProfile. Logging in to AWS SSO."
    Invoke-WithErrorHandling aws sso login --profile "$AWSProfile"
    if ($LASTEXITCODE -ne 0) {
      Write-Error "Could not find a valid profile $AWSProfile."
      exit 1
    }
  }
